...
JointTargetBridge::JointTargetBridge(std::array<float, 6> joints, std::array<float, 6> ext_joints) : joints_(joints), ext_joints_(ext_joints) {}
...
RapidRobJoint JointTargetBridge::to_rapid() const {
    RapidRobJoint bin;
    std::memcpy(&bin.j1, &joints_[0], 4);
    std::memcpy(&bin.j2, &joints_[1], 4);
    std::memcpy(&bin.j3, &joints_[2], 4);
